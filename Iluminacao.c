...
void display()
{
    ...
    glEnable(GL_LIGHT0);
    glEnable(GL_COLOR_MATERIAL); // Habilitando cor

    float globalAmb[] = {0.1f, 0.1f, 0.1f, 1.f}; // intensidade da luz ambiente
    glLightModelfv(GL_LIGHT_MODEL_AMBIENT, globalAmb); 

    float light0[4][4] = {
        {0.1f, 0.1f, 0.1f, 1.f}, // ambiente
        {0.6f, 0.6f, 0.6f, 1.f}, // difusa
        {1.f, 1.f, 1.f, 1.f}, // specular
        {0.f, 0.f, 1.f, 1.f} // posição da luz
    };

    glLightfv(GL_LIGHT0, GL_AMBIENT, &light0[0][0]); // luz, caracteristica, posiçao
    glLightfv(GL_LIGHT0, GL_DIFFUSE, &light0[1][0]); // luz, caracteristica, posiçao
    glLightfv(GL_LIGHT0, GL_SPECULAR, &light0[2][0]); // luz, caracteristica, posiçao
    glLightfv(GL_LIGHT0, GL_POSITION, &light0[3][0]); // luz, caracteristica, posiçao
...
int main(int argc, char **argv)
{
    ...
    glutInitWindowPosition(960, 640);
